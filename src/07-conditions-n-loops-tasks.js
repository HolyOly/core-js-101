...
function findFirstSingleChar(str) {
  ...
  const arrOfStr = str.split('').reduce((acc, item) => {
    if (item in acc) {
      acc[item] += 1;
    } else {
      acc[item] = 1;
  ...
  for (let i = 0; i < arr.length; i += 1) {
    arrInd.push(strArr.indexOf(arr[i]));
  }
  return strArr[Math.min(...arrInd)];
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${parStart}${start}, ${end}${parEnd}`;
}
...
function getDigitalRoot(num) {
  // throw new Error('Not implemented');
  const val = num.toString().split('').map((item) => +item).reduce((acc, item) => acc + item, 0);
  if (val > 10) {
